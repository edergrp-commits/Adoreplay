...
import { Users2, GraduationCap, Heart, Library, Layout, Mic, Play, Star, Clock, CheckCircle2, ArrowLeft, Trophy, Users, Loader2 } from 'lucide-react';
...
export default function MasterclassDetailPage() {
  ...
  const [dbMasterclass, setDbMasterclass] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const q = query(collection(db, 'masterclasses'), where('slug', '==', slug), limit(1));
        const querySnapshot = await getDocs(q);
        
        if (!querySnapshot.empty) {
          const doc = querySnapshot.docs[0];
          setDbMasterclass({ id: doc.id, ...doc.data() });
        }
      } catch (error) {
  ...
  const masterclass = dbMasterclass;

  if (!masterclass) return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center text-center p-8">
      <h2 className="text-2xl font-black text-white mb-4 uppercase">Masterclass não encontrada</h2>
      <Link to="/masterclass" className="text-primary font-bold hover:underline">Voltar para Masterclasses</Link>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[60vh] w-full overflow-hidden">
        <img 
          src={masterclass.bannerURL || masterclass.image} 
        ...
        <div className="relative h-full container mx-auto px-8 flex flex-col justify-end pb-16">
          <Link to="/masterclass" className="flex items-center gap-2 text-primary font-bold text-xs uppercase tracking-widest mb-8 hover:translate-x-[-4px] transition-transform">
            <ArrowLeft size={16} />
            ...
            <div className="flex flex-wrap items-center gap-4 mb-6">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-primary/20 backdrop-blur-md border border-primary/30 flex items-center justify-center">
                  {(() => {
                    const Icon = masterclass.icon || GraduationCap;
                    return <Icon size={24} className="text-primary" />;
                  })()}
                </div>
                <span className="text-primary font-black uppercase tracking-[0.3em] text-xs">Masterclass Especializada</span>
              </div>
              
              <FavoriteButton contentId={masterclass.id} showText={true} />
            </div>
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {masterclass.topics.map((topic: string, i: number) => (
              ...
              <div className="p-8 rounded-2xl bg-surface-container-high border border-primary/30 shadow-[0_20px_50px_rgba(24,165,179,0.1)]">
                ...
                <div className="mt-8 pt-8 border-t border-white/5 space-y-4">
                  ...
                  <div className="flex items-center gap-3 text-on-surface-variant text-xs font-bold uppercase tracking-wide">
                    ...
                    <span>Instrutores Renomados</span>
                  </div>
                </div>
              </div>
            </div>
